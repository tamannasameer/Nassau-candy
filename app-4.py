...
with tab2:
    ...
    fig_map.add_trace(go.Choropleth(
        locations=state_agg["State_Abbrev"],
        z=state_agg[z_col],
        locationmode="USA-states",
        colorscale=color_scale,
        hovertext=state_agg["hover_text"],
        hoverinfo="text",
        marker_line_color="#6B7280",
        marker_line_width=0.5,
        colorbar=dict(
            title=dict(text=color_label, font=dict(color="#D1D5DB")),
    ...
    fig_map.update_layout(
        ...
        font_color="#D1D5DB",
        title=dict(text=f"US State-Level Shipping Performance — {map_metric}",
...
with tab4:
    ...
    if len(dd_df) == 0:
        ...
        st.dataframe(disp.reset_index(drop=True), height=360, use_container_width=True)    header    {visibility: hidden;}
...
with tab2:
    ...
    fig_map.update_layout(
        paper_bgcolor="rgba(0,0,0,0)",
        geo=dict(bgcolor="rgba(0,0,0,0)", lakecolor="#1E1E2E", landcolor="#2A2A3E",
                 showlakes=True, showland=True, countrycolor="#4B5563",
                 subunitcolor="#6B7280"),
        font_color="#D1D5DB",
        legend=dict(
            bgcolor="rgba(30,30,46,0.9)", bordercolor="#3F3F5F",
        ...
        height=520,
        coloraxis_colorbar=dict(
            title=color_label, tickfont_color="#D1D5DB",
